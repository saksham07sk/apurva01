  ...
  std::weak_ptr<agv::FleetUpdateHandle> fleet_handle)
{
  ...
  // Initialize lane closure publisher
  mgr->_lane_closure_pub = mgr->_context->node()->create_publisher<rmf_fleet_msgs::msg::LaneRequest>(
  ...
  return [w = weak_from_this()](rmf_task::Phase::ConstSnapshotPtr snapshot)
    {
      ...
      // 🚀 REAL-TIME PHASE INFO EXTRACTION
      if (snapshot)
      {
        const auto& tag = snapshot->tag();
        const auto& header = tag->header();
        const auto phase_id = tag->id();
        const auto status = snapshot->final_event()->status();
        
       /*  // 🎯 CUSTOM CONDITIONS FOR PHASE COMPLETION
        if (phase_id == 1 && header.category() == "Pick up" && static_cast<int>(status) == 10)
        {
          std::cout << "🚀 Pick-up phase completed! Closing delivery route lanes..." << std::endl;
          
          auto lane_request_msg = std::make_unique<rmf_fleet_msgs::msg::LaneRequest>();
          lane_request_msg->fleet_name = "tinyRobot";
          lane_request_msg->close_lanes = {8, 9, 10, 11};
          lane_request_msg->open_lanes = {};
          
          self->_lane_closure_pub->publish(std::move(lane_request_msg));
          std::cout << "✅ Published lane closure request for lanes {8, 9, 10, 11}" << std::endl;
        }
        
        if (phase_id == 2 && header.category() == "Drop off" && static_cast<int>(status) == 10)
        {
          std::cout << "🚀🚀 Drop-off phase completed! Reopening delivery route lanes..." << std::endl;
          
          auto lane_request_msg = std::make_unique<rmf_fleet_msgs::msg::LaneRequest>();
          lane_request_msg->fleet_name = "tinyRobot";
          lane_request_msg->close_lanes = {};
          lane_request_msg->open_lanes = {8, 9, 10, 11};
          
          self->_lane_closure_pub->publish(std::move(lane_request_msg));
          std::cout << "✅ Published lane opening request for lanes {8, 9, 10, 11}" << std::endl;
        }*/
      }
