...
void Polygon::from_yaml(const YAML::Node& data, const Type polygon_type)
{
  ...
  if (type == ZONE)
  {
    ...
    if (data["id"])
      attributes.id = data["id"].as<string>();
    if (data["name"])
      attributes.name = data["name"].as<string>();
    if (attributes.name.empty())
      attributes.name = attributes.id;

    const auto load_index_sequence =
      [](const YAML::Node& node, std::vector<int>& destination)
      {
        if (!node || !node.IsSequence())
          return;

        for (const auto& value : node)
          destination.push_back(value.as<int>());
      };

    load_index_sequence(data["waypoints"], attributes.waypoints);
    load_index_sequence(data["lanes"], attributes.lanes);
...
YAML::Node Polygon::to_yaml() const
{
  ...
  if (type == ZONE && zone.has_value())
  {
    const auto& attributes = zone.value();
    if (!attributes.id.empty())
      y["id"] = attributes.id;
    if (!attributes.name.empty())
      y["name"] = attributes.name;

    if (!attributes.waypoints.empty())
    {
      for (const auto& waypoint_idx : attributes.waypoints)
        y["waypoints"].push_back(waypoint_idx);
      y["waypoints"].SetStyle(YAML::EmitterStyle::Flow);
    }

    if (!attributes.lanes.empty())
    {
      for (const auto& lane_idx : attributes.lanes)
        y["lanes"].push_back(lane_idx);
      y["lanes"].SetStyle(YAML::EmitterStyle::Flow);
    }
  }
...
{
  // create required parameters if they don't exist yet on this polygon
  if (type == FLOOR)
  {
  ...
  {
    // Ensure we have a string parameter slot for zone_color so that
    // Editor::property_editor_cell_changed can update it and Level::draw_polygons
    // can read it to determine the fill color.
    create_param_if_needed(
      "zone_color",
      Param::STRING,
      std::string("#4D99FF"));  // default blue
  }
}

bool Polygon::has_zone() const
{
  return type == ZONE && zone.has_value();
}

Polygon::ZoneAttributes& Polygon::ensure_zone()
{
  if (!zone.has_value())
  {
    ZoneAttributes attributes;
...
const Polygon::ZoneAttributes* Polygon::get_zone() const
{
  if (!has_zone())
    return nullptr;
