...
: QMainWindow()
{
  ...
  create_tool_button(TOOL_ADD_ZONE, ":icons/roi.svg", "Add zone (Z)");
...
void Editor::mouse_event(const MouseType t, QMouseEvent* e)
{
  ...
  switch (tool_id)
  {
    ...
    case TOOL_ADD_ZONE:     mouse_add_zone(t, e, p); break;
...
void Editor::keyPressEvent(QKeyEvent* e)
{
  switch (e->key())
  {
    ...
    case Qt::Key_Z:
      clear_current_tool_buffer();
      tool_button_group->button(TOOL_ADD_ZONE)->click();
      break;
    case Qt::Key_B:
...
void Editor::populate_property_editor(const Polygon& polygon)
{
  ...
  const bool is_zone = polygon.type == Polygon::ZONE;
  // Zone rows: zone_id, zone_name, zone_color, zone_waypoints, zone_lanes
  const int base_rows = is_zone ? 5 : 0;
  property_editor->setRowCount(base_rows + polygon.params.size());

  int row = 0;
  if (is_zone)
  {
    const auto* zone = polygon.get_zone();
    const QString zone_id =
      zone ? QString::fromStdString(zone->id) : QString();
    const QString zone_name =
      zone ? QString::fromStdString(zone->name) : QString();

    const auto join_indices =
      [](const std::vector<int>& indices) -> QString
      {
        QString joined;
        for (std::size_t i = 0; i < indices.size(); ++i)
        {
          if (i > 0)
            joined += ", ";
          joined += QString::number(indices[i]);
        }
        return joined;
      };

    const QString waypoint_string =
      zone ? join_indices(zone->waypoints) : QString();
    const QString lane_string =
      zone ? join_indices(zone->lanes) : QString();

    property_editor_set_row(row++, "zone_id", zone_id, true);
    property_editor_set_row(row++, "zone_name", zone_name, true);
    
    // Zone color parameter
    QString zone_color_str;
    auto color_param_it = polygon.params.find("zone_color");
    if (color_param_it != polygon.params.end() &&
        color_param_it->second.type == Param::STRING &&
        !color_param_it->second.value_string.empty())
    {
      zone_color_str = QString::fromStdString(color_param_it->second.value_string);
    }
    else
    {
      zone_color_str = "#4D99FF";  // Default blue color in hex
    }

    const int zone_color_row = row;
    property_editor_set_row(row++, "zone_color", zone_color_str, true);

    // Create a combo box for color selection
    QComboBox* zone_color_combo = new QComboBox(property_editor);
    zone_color_combo->setEditable(false);
    ...
    {
      zone_color_combo->addItem(
        QString::fromUtf8(preset.label),
        QString::fromUtf8(preset.value));
    }
    // Show a friendly label but store the raw color code in the item data.
    // If the current zone color matches one of the presets, select it;
    // otherwise, add a custom entry.
    int matched_index = -1;
    for (int i = 0; i < zone_color_combo->count(); ++i)
    {
      if (zone_color_combo->itemData(i).toString() == zone_color_str)
      {
        matched_index = i;
        break;
      }
    ...
    if (matched_index >= 0)
    {
      zone_color_combo->setCurrentIndex(matched_index);
    }
    else
    {
      zone_color_combo->addItem(zone_color_str, zone_color_str);
      zone_color_combo->setCurrentIndex(zone_color_combo->count() - 1);
    }

    property_editor->setCellWidget(zone_color_row, 1, zone_color_combo);

    connect(
      zone_color_combo,
      &QComboBox::currentTextChanged,
      this,
      [this, zone_color_row, zone_color_combo](const QString&)
      {
        if (auto* item = property_editor->item(zone_color_row, 1))
        {
          // Always write the raw color code (item data) into the cell so that
          // Level::draw_polygons() receives a parseable value like "#FF0000".
          const QString value =
            zone_color_combo->currentData().toString().trimmed();
          item->setText(value);
          property_editor_cell_changed(zone_color_row, 1);
        }
      });

    property_editor_set_row(row++, "zone_waypoints", waypoint_string, true);
    property_editor_set_row(row++, "zone_lanes", lane_string, true);
  }
...
void Editor::property_editor_cell_changed(int row, int column)
{
  ...
  for (auto& p : building.levels[level_idx].polygons)
  {
    ...
    if (p.type == Polygon::ZONE)
    {
      auto& zone = p.ensure_zone();
      if (name == "zone_id")
        zone.id = value;
      else if (name == "zone_name")
        zone.name = value;
      else if (name == "zone_color")
        p.set_param("zone_color", value);
      else if (name == "zone_waypoints")
        zone.waypoints = parse_index_list(value);
      else if (name == "zone_lanes")
        zone.lanes = parse_index_list(value);
      else
    ...
    // Redraw the scene so polygon/zone changes (including zone_color)
  ...
  if (t == MOUSE_PRESS)
  {
    ...
    else if (e->buttons() & Qt::RightButton)
    {
      ...
      if (mouse_motion_polygon_vertices.size() >= 3)
      {
        ...
        if (polygon_type == Polygon::ZONE)
        {
          auto& zone = polygon.ensure_zone();
          if (zone.id.empty())
            zone.id = building.generate_zone_id();
          if (zone.name.empty())
            zone.name = zone.id;
        }
        ...
        // Auto-populate waypoints and lanes for zones
        if (polygon_type == Polygon::ZONE)
        {
          Level* level = active_level();
          if (level != nullptr)
          {
            level->auto_populate_zone_waypoints_and_lanes(polygon);
          }
        }
...
void Editor::set_mode(const EditorModeId _mode, const QString& mode_string)
{
  ...
  // scenario tools
  set_tool_visibility(TOOL_ADD_ZONE, mode == MODE_SCENARIO);
