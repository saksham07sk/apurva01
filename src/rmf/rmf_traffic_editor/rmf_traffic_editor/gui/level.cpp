  ...
  const CoordinateSystem& coordinate_system)
{
  ...
  if (_data["zones"] && _data["zones"].IsSequence())
  {
    const YAML::Node& yz = _data["zones"];
    for (YAML::const_iterator it = yz.begin(); it != yz.end(); ++it)
    {
      Polygon p;
      p.from_yaml(*it, Polygon::ZONE);
      polygons.push_back(p);
    }
  }
  ...
  for (const auto& polygon : polygons)
  {
    switch (polygon.type)
    {
      ...
      case Polygon::HOLE:
        ...
        break;
      case Polygon::ZONE:
        y["zones"].push_back(polygon.to_yaml());
        break;
      default:
...
void Level::draw_polygons(QGraphicsScene* scene) const
{
  ...
  const QBrush zone_brush(QColor::fromRgbF(0.3, 0.6, 1.0, 0.35));
  ...
  // finally draw zones so they appear on top of floors
  for (const auto& polygon : polygons)
  {
    if (polygon.type == Polygon::ZONE)
    {
      // Get zone color from parameters, or use default blue
      QBrush zone_brush_to_use = zone_brush;
      ...
      {
        const std::string& color_str = color_param_it->second.value_string;
        if (!color_str.empty())
        {
          QColor zone_color;
          if (color_str[0] == '#')
          {
            ...
            zone_color = QColor(QString::fromStdString(color_str));
          }
        else
        {
          // Try to parse as comma-separated RGB or RGBA values
          ...
          if (rgb.size() >= 3)
          {
            double alpha = rgb.size() >= 4 ? rgb[3] : 0.35;
            zone_color = QColor::fromRgbF(
              std::clamp(rgb[0], 0.0, 1.0),
              std::clamp(rgb[1], 0.0, 1.0),
              std::clamp(rgb[2], 0.0, 1.0),
              std::clamp(alpha, 0.0, 1.0));
          }
        }

          // If we parsed a valid color, enforce a reasonable transparency
          // so that zones don't completely hide the floor.
          if (zone_color.isValid())
          {
            if (zone_color.alphaF() <= 0.0 || zone_color.alphaF() > 1.0)
              zone_color.setAlphaF(0.35);

            zone_brush_to_use = QBrush(zone_color);
          }
        }
      }
      draw_polygon(scene, zone_brush_to_use, polygon);
    }
  }
  ...
  const double y)
{
  ...
  // prefer selecting zones over floors when present
  for (Polygon* p : containing_polygons)
  {
...
void Level::auto_populate_zone_waypoints_and_lanes(Polygon& zone_polygon) const
{
  if (zone_polygon.type != Polygon::ZONE)
    return;

  auto& zone = zone_polygon.ensure_zone();
  zone.waypoints.clear();
  zone.lanes.clear();

  // Ensure boundary vertices are listed first in the waypoint list
  zone.waypoints = zone_polygon.vertices;

  // Build QPolygonF from the boundary vertices for point-in-polygon checks
  QVector<QPointF> polygon_vertices;
  polygon_vertices.reserve(zone_polygon.vertices.size());
  for (const auto& vertex_idx : zone_polygon.vertices)
  {
  ...
  const std::set<int> boundary_vertices(
    zone_polygon.vertices.begin(),
    zone_polygon.vertices.end());

  const auto point_on_boundary_edge =
    [&](const QPointF& p) -> bool
    {
      const double tolerance = 1e-8;
      const int polygon_vertex_count = static_cast<int>(zone_polygon.vertices.size());
      for (int i = 0; i < polygon_vertex_count; ++i)
      {
        const int start_idx = zone_polygon.vertices[i];
        const int end_idx = zone_polygon.vertices[(i + 1) % polygon_vertex_count];

        if (start_idx < 0 || start_idx >= static_cast<int>(vertices.size()) ||
          end_idx < 0 || end_idx >= static_cast<int>(vertices.size()))
          continue;

        const Vertex& a = vertices[start_idx];
        const Vertex& b = vertices[end_idx];
        ...
        const double len_sq = (bx - ax) * (bx - ax) + (by - ay) * (by - ay);
  ...
  // Keep a set so we can append interior vertices without duplicates
  std::set<int> zone_waypoint_set(
    zone_polygon.vertices.begin(),
    zone_polygon.vertices.end());

  const auto vertex_inside_zone =
    [&](const int idx) -> bool
    {
      if (idx < 0 || idx >= static_cast<int>(vertices.size()))
        return false;

      if (boundary_vertices.find(idx) != boundary_vertices.end())
        return true;

      const Vertex& v = vertices[idx];
      const QPointF point(v.x, v.y);
      if (qpolygon.containsPoint(point, Qt::OddEvenFill))
        return true;

      return point_on_boundary_edge(point);
    };

  // Append any interior vertices (waypoints) that fall inside the polygon
  for (std::size_t i = 0; i < vertices.size(); ++i)
  {
    const int idx = static_cast<int>(i);
    if (zone_waypoint_set.find(idx) != zone_waypoint_set.end())
      continue;

    if (vertex_inside_zone(idx))
    {
      zone_waypoint_set.insert(idx);
      zone.waypoints.push_back(idx);
    }
  }

  // Lanes are included only when both endpoints lie inside the zone polygon.
  int lane_index = 0;
  for (const auto& edge : edges)
  {
    if (edge.type != Edge::LANE)
    ...
    const bool start_inside = vertex_inside_zone(edge.start_idx);
    const bool end_inside = vertex_inside_zone(edge.end_idx);

    if (start_inside && end_inside)
      zone.lanes.push_back(lane_index);
