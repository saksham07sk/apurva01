...
class Level:
    ...
    def parse_yaml(
        ...
        if 'zones' in yaml_node:
            for zone_yaml in yaml_node['zones']:
                zone = {
                    'id': zone_yaml.get('id', ''),
                    'name': zone_yaml.get('name', ''),
                    'vertices': zone_yaml.get('vertices', []),
                    'waypoints': zone_yaml.get('waypoints', []),
                    'lanes': zone_yaml.get('lanes', []),
                    'parameters': {}
                }
                if 'parameters' in zone_yaml and zone_yaml['parameters']:
                    for param_name, param_yaml in \
                            zone_yaml['parameters'].items():
                        zone['parameters'][param_name] = \
                            ParamValue(param_yaml)
    ...
    def to_yaml(self, coordinate_system):
        ...
        if self.zones:
            ...
            for zone in self.zones:
                zone_yaml = {}
                if zone.get('id'):
                    zone_yaml['id'] = zone['id']
                if zone.get('name'):
                    zone_yaml['name'] = zone['name']
                zone_yaml['vertices'] = zone.get('vertices', [])
                if zone.get('waypoints'):
                    zone_yaml['waypoints'] = zone['waypoints']
                if zone.get('lanes'):
                    zone_yaml['lanes'] = zone['lanes']
                if zone['parameters']:
                    zone_yaml['parameters'] = {}
                    for param_name, param_value in \
                            zone['parameters'].items():
                        zone_yaml['parameters'][param_name] = \
                            param_value.to_yaml()
                y['zones'].append(zone_yaml)
        return y
