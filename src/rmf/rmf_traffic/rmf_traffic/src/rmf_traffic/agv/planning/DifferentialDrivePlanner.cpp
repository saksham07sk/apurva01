...
public:
  ...
  void expand_start(const SearchNodePtr& top, SearchQueue& queue) const
  {
    ...
    // Note: entry_event_cost is calculated but not used in expand_start
    // It's kept for consistency with expand() method signature
    ...
    SearchQueue& queue) const
  {
    ...
    // ===========================================================================
    // ZONE FILTERING: Skip traversals to waypoints outside zones
    // ===========================================================================
    auto& zone_helper = ZonePlannerHelper::get_instance();
    if (zone_helper.is_enabled())
    {
      const auto* integration = zone_helper.get_integration();
      if (integration && !integration->is_waypoint_in_zone(next_waypoint_index))
      {
        // Skip this traversal - waypoint is outside zone
        return;
      }
    }
...
{
  const Graph& full_graph = _config.graph();
  
  // ===========================================================================
  // AUTOMATIC ZONE INITIALIZATION
  // Try to initialize zones automatically if not already initialized
  // ===========================================================================
  if (!ZoneInitialization::is_initialized())
  {
    // Try to find building.yaml in common locations
    std::vector<std::string> possible_paths;
    
    // 1. Check environment variable (highest priority)
    const char* env_path = std::getenv("RMF_BUILDING_MAP_PATH");
    if (env_path)
    {
      possible_paths.push_back(env_path);
    }
    
    // 2. Try common workspace-relative paths
    const char* workspace = std::getenv("COLCON_PREFIX_PATH");
    if (workspace)
    {
      std::string ws_str(workspace);
      // Go up to workspace root: install/package -> install -> workspace
      size_t pos = ws_str.find("/install/");
      if (pos != std::string::npos)
      {
        std::string ws_root = ws_str.substr(0, pos);
        possible_paths.push_back(ws_root + "/src/demonstrations/rmf_demos/rmf_demos_maps/maps/four_d/four_d.building.yaml");
        possible_paths.push_back(ws_root + "/src/demonstrations/rmf_demos/rmf_demos_maps/maps/office/office.building.yaml");
        possible_paths.push_back(ws_root + "/src/demonstrations/rmf_demos/rmf_demos_maps/maps/hotel/hotel.building.yaml");
      }
    }
    
    // 3. Try current directory and common locations
    possible_paths.push_back("building.yaml");
    possible_paths.push_back("../building.yaml");
    possible_paths.push_back("../../building.yaml");
    
    // Try each path (check if file exists by trying to open it)
    ...
    for (const auto& path : possible_paths)
    {
      ...
      {
        test_file.close();
        std::cout << "[DifferentialDrivePlanner] 🔍 Found building.yaml at: " << path << std::endl;
        zones_initialized = ZoneInitialization::initialize_from_building_map(
          full_graph,
          path,
          "L1"  // Default level - adjust if needed
        );
        if (zones_initialized)
        {
          std::cout << "[DifferentialDrivePlanner] ✅ Zones initialized automatically" << std::endl;
          break;
        }
      }
    }
    ...
    if (!zones_initialized)
    {
      std::cout << "[DifferentialDrivePlanner] ℹ️  Zone-based planning not initialized (zones will be auto-detected if building.yaml found)" << std::endl;
      std::cout << "[DifferentialDrivePlanner]    To manually enable zones, call:" << std::endl;
      std::cout << "[DifferentialDrivePlanner]    ZoneInitialization::initialize_from_building_map(graph, \"path/to/building.yaml\", \"L1\")" << std::endl;
      std::cout << "[DifferentialDrivePlanner]    Or set RMF_BUILDING_MAP_PATH environment variable" << std::endl;
    }
  }
  else
  {
    std::cout << "[DifferentialDrivePlanner] ✅ Zones already initialized" << std::endl;
  }
  
  _supergraph = Supergraph::make(
    Graph::Implementation::get(full_graph),
  ...
  Planner::Options options) const
{
  ...
  // ===========================================================================
  // ZONE-BASED PLANNING: Validate tasks BEFORE starting planning
  // ===========================================================================
  auto& zone_helper = ZonePlannerHelper::get_instance();
  const bool zones_enabled = zone_helper.is_enabled();
  
  // Only enforce zone validation if zones are actually enabled
  // If zones aren't enabled, allow planning on full map (backward compatibility)
  if (zones_enabled)
  {
    // Validate all start/goal waypoints are in zones (or are charging stations)
    for (const auto& start : starts)
    {
      const std::size_t start_waypoint_full = start.waypoint();
      const std::size_t goal_waypoint_full = goal.waypoint();
      
      // Use zone_helper validation which handles charging stations outside zones
      if (!zone_helper.validate_task(start_waypoint_full, goal_waypoint_full))
      {
        std::cerr << "[DifferentialDrivePlanner::initiate] ❌ TASK REJECTED: Waypoints outside zones" << std::endl;
        std::cerr << "[DifferentialDrivePlanner::initiate]   Start waypoint: " << start_waypoint_full << std::endl;
        std::cerr << "[DifferentialDrivePlanner::initiate]   Goal waypoint: " << goal_waypoint_full << std::endl;
        std::cout << "[DifferentialDrivePlanner::initiate] ========================================\n" << std::endl;
        
        // Ensure state is properly initialized before returning
        // This prevents crashes when the task planner tries to use this state
        state.issues.disconnected = true;
        state.ideal_cost = std::nullopt;  // Ensure ideal_cost is explicitly set to nullopt
        
        // Ensure internal state is initialized (even if empty) to prevent crashes
        // The state.internal was already initialized at the start of the function,
        // but we ensure it's still valid here
        if (!state.internal)
        {
          using InternalState = ScheduledDifferentialDriveExpander::InternalState;
          state.internal = rmf_utils::make_derived_impl<State::Internal, InternalState>();
        }
        
        // Ensure the internal queue is empty (it should be, but be explicit)
        auto& internal = static_cast<ScheduledDifferentialDriveExpander::InternalState&>(*state.internal);
        while (!internal.queue.empty())
        {
          internal.queue.pop();
        }
        
        return state;  // Return immediately - don't create expander
      }
    }
  }
...
{
  const auto& goal = state.conditions.goal;
  const auto& full_graph = _supergraph->original();  // Full graph (for visualization)
  const auto& traits = _supergraph->traits();
  ...
  const auto& start = state.conditions.starts[0];
  const std::size_t start_waypoint_full = start.waypoint();  // Full graph index
  const std::size_t goal_waypoint_full = goal.waypoint();    // Full graph index
  
  // ===========================================================================
  // ZONE-BASED PLANNING: Check if zones are enabled and handle accordingly
  // ===========================================================================
  auto& zone_helper = ZonePlannerHelper::get_instance();
  const bool zones_enabled = zone_helper.is_enabled();
  
  std::cout << "\n[DifferentialDrivePlanner] ========================================" << std::endl;
  std::cout << "[DifferentialDrivePlanner] 🗺️  PATH PLANNING REQUEST" << std::endl;
  std::cout << "[DifferentialDrivePlanner]   Start waypoint (full): " << start_waypoint_full << std::endl;
  std::cout << "[DifferentialDrivePlanner]   Goal waypoint (full): " << goal_waypoint_full << std::endl;
  std::cout << "[DifferentialDrivePlanner]   Full graph: " << full_graph.waypoints.size() 
            << " waypoints, " << full_graph.lanes.size() << " lanes" << std::endl;
  std::cout << "[DifferentialDrivePlanner]   Zone planning: " << (zones_enabled ? "ENABLED" : "DISABLED") << std::endl;
  std::cout << "[DifferentialDrivePlanner] ----------------------------------------" << std::endl;
  
  // Validate waypoint indices are within full graph bounds
  if (start_waypoint_full >= full_graph.waypoints.size())
  {
    std::cerr << "[DifferentialDrivePlanner] ❌ ERROR: Start waypoint " 
              << start_waypoint_full << " is out of bounds!" << std::endl;
    std::cerr << "[DifferentialDrivePlanner]   Full graph has only " 
              << full_graph.waypoints.size() << " waypoints" << std::endl;
    std::cout << "[DifferentialDrivePlanner] ========================================\n" << std::endl;
    state.issues.disconnected = true;
    return std::nullopt;
  }
  
  if (goal_waypoint_full >= full_graph.waypoints.size())
  {
    std::cerr << "[DifferentialDrivePlanner] ❌ ERROR: Goal waypoint " 
              << goal_waypoint_full << " is out of bounds!" << std::endl;
    std::cerr << "[DifferentialDrivePlanner]   Full graph has only " 
              << full_graph.waypoints.size() << " waypoints" << std::endl;
    std::cout << "[DifferentialDrivePlanner] ========================================\n" << std::endl;
    state.issues.disconnected = true;
  ...
  // Zone-based task validation: Reject tasks outside zones (unless charging stations)
  if (zones_enabled)
  {
    if (!zone_helper.validate_task(start_waypoint_full, goal_waypoint_full))
    {
      std::cerr << "[DifferentialDrivePlanner] ❌ TASK REJECTED: Waypoints outside zones" << std::endl;
      std::cout << "[DifferentialDrivePlanner] ========================================\n" << std::endl;
      state.issues.disconnected = true;
      return std::nullopt;
    }
  ...
  // Get planning graph (filtered if zones enabled, full otherwise)
  const auto& planning_graph = zone_helper.get_planning_graph(full_graph);
  
  // Translate waypoint indices from full graph to filtered graph (if zones enabled)
  std::size_t start_waypoint_planning = start_waypoint_full;
  std::size_t goal_waypoint_planning = goal_waypoint_full;
  
  if (zones_enabled)
  {
    auto start_filtered = zone_helper.translate_to_filtered(start_waypoint_full);
    auto goal_filtered = zone_helper.translate_to_filtered(goal_waypoint_full);
    
    if (!start_filtered.has_value())
    {
      std::cerr << "[DifferentialDrivePlanner] ❌ ERROR: Start waypoint " 
                << start_waypoint_full << " not in filtered graph" << std::endl;
      std::cout << "[DifferentialDrivePlanner] ========================================\n" << std::endl;
      state.issues.disconnected = true;
      return std::nullopt;
    }
    
    if (!goal_filtered.has_value())
    {
      std::cerr << "[DifferentialDrivePlanner] ❌ ERROR: Goal waypoint " 
                << goal_waypoint_full << " not in filtered graph" << std::endl;
      std::cout << "[DifferentialDrivePlanner] ========================================\n" << std::endl;
      state.issues.disconnected = true;
      return std::nullopt;
    }
    
    start_waypoint_planning = start_filtered.value();
    goal_waypoint_planning = goal_filtered.value();
    
    std::cout << "[DifferentialDrivePlanner]   Start waypoint (filtered): " << start_waypoint_planning << std::endl;
    std::cout << "[DifferentialDrivePlanner]   Goal waypoint (filtered): " << goal_waypoint_planning << std::endl;
    std::cout << "[DifferentialDrivePlanner]   Planning graph: " << planning_graph.waypoints.size() 
              << " waypoints, " << planning_graph.lanes.size() << " lanes" << std::endl;
  }
  
  std::cout << "[DifferentialDrivePlanner] ✅ Waypoints validated" << std::endl;
  
  // ===========================================================================
  // CUOPT ROUTING: Use cuOpt distance_engine for GPU-accelerated shortest path
  // Uses filtered graph for planning (zone waypoints/lanes only)
  // ===========================================================================
  auto cuopt_result = CuOptRouter::find_shortest_path(
    start_waypoint_planning,
    goal_waypoint_planning,
    planning_graph  // Use filtered graph for planning
  );
  
  // If cuOpt finds no path, the goal is unreachable
  if (!cuopt_result.has_value() || cuopt_result->path.empty())
  {
    std::cout << "[DifferentialDrivePlanner] ❌ NO PATH FOUND" << std::endl;
    std::cout << "[DifferentialDrivePlanner]   Start and goal may be in disconnected zones" << std::endl;
    std::cout << "[DifferentialDrivePlanner] ========================================\n" << std::endl;
    state.issues.disconnected = true;
    return std::nullopt;
  }
  
  std::cout << "[DifferentialDrivePlanner] ✅ PATH FOUND" << std::endl;
  std::cout << "[DifferentialDrivePlanner]   Path length: " << cuopt_result->path.size() << " waypoints" << std::endl;
  std::cout << "[DifferentialDrivePlanner]   Total cost: " << cuopt_result->total_cost << " meters" << std::endl;
  
  // Translate path indices from filtered graph back to full graph (if zones enabled)
  std::vector<std::size_t> path_full_indices;
  if (zones_enabled)
  {
    for (std::size_t filtered_idx : cuopt_result->path)
    {
      auto full_idx = zone_helper.translate_to_full(filtered_idx);
      if (full_idx.has_value() && full_idx.value() < full_graph.waypoints.size())
      {
        path_full_indices.push_back(full_idx.value());
      }
      else
      {
        std::cerr << "[DifferentialDrivePlanner] ❌ ERROR: Could not translate filtered index " 
                  << filtered_idx << " to valid full graph index" << std::endl;
        if (full_idx.has_value())
        {
          std::cerr << "[DifferentialDrivePlanner]   Translated index " << full_idx.value() 
                    << " is out of bounds (full graph has " << full_graph.waypoints.size() << " waypoints)" << std::endl;
        }
        std::cerr << "[DifferentialDrivePlanner]   This indicates a bug in zone filtering - rejecting path" << std::endl;
        state.issues.disconnected = true;
        return std::nullopt;
      }
    }
    std::cout << "[DifferentialDrivePlanner]   Path translated to full graph indices (" 
              << path_full_indices.size() << " waypoints)" << std::endl;
  }
  else
  {
    path_full_indices = cuopt_result->path;
  }
  
  // Validate path is not empty
  if (path_full_indices.empty())
  {
    std::cerr << "[DifferentialDrivePlanner] ❌ ERROR: Path is empty after translation" << std::endl;
    state.issues.disconnected = true;
    return std::nullopt;
  }
  
  // Validate all waypoint indices are within bounds
  for (std::size_t wp_idx : path_full_indices)
  {
    if (wp_idx >= full_graph.waypoints.size())
    {
      std::cerr << "[DifferentialDrivePlanner] ❌ ERROR: Waypoint index " << wp_idx 
                << " is out of bounds (full graph has " << full_graph.waypoints.size() << " waypoints)" << std::endl;
      state.issues.disconnected = true;
      return std::nullopt;
    }
  ...
  // Use full graph indices for path (for execution and visualization)
  const auto& path = path_full_indices;
  
  // Get robot kinematic parameters
  const double linear_velocity = traits.linear().get_nominal_velocity();
  const double angular_velocity = traits.rotational().get_nominal_velocity();
  
  // Build trajectory and waypoints from cuOpt path
  // Use full graph for waypoint information (for visualization and execution)
  std::vector<Route> routes;
  std::vector<Plan::Waypoint> plan_waypoints;
  
  Time current_time = start.time();
  const std::string& map_name = full_graph.waypoints[path[0]].get_map_name();
  
  // Create trajectory
  Trajectory trajectory;
  
  // Start position (use full graph for location)
  Eigen::Vector3d current_pos;
  current_pos[0] = full_graph.waypoints[path[0]].get_location().x();
  current_pos[1] = full_graph.waypoints[path[0]].get_location().y();
  current_pos[2] = start.orientation();
  
  trajectory.insert(current_time, current_pos, Eigen::Vector3d::Zero());
  
  // Add start waypoint
  Plan::Checkpoints start_cp;
  start_cp.push_back(Plan::Checkpoint{0, 0});
  plan_waypoints.push_back(Plan::Waypoint::Implementation::make(
    current_pos, current_time, path[0], 
    std::vector<std::size_t>{}, std::vector<Plan::Progress>{},
    start_cp, nullptr, Dependencies{}
  ));
  
  // Build path through all waypoints using cuOpt route
  // Use full graph for waypoint information (for visualization and execution)
  double total_cost = 0.0;
  for (std::size_t i = 1; i < path.size(); ++i)
  {
    const auto& prev_wp = full_graph.waypoints[path[i-1]];
    const auto& curr_wp = full_graph.waypoints[path[i]];
    
    const Eigen::Vector2d prev_loc = prev_wp.get_location();
    const Eigen::Vector2d curr_loc = curr_wp.get_location();
    
    // Calculate distance (like cuopt_pickup_delivery.cpp)
    const double dx = curr_loc.x() - prev_loc.x();
    const double dy = curr_loc.y() - prev_loc.y();
    ...
    // Rotation time
    double yaw_diff = std::abs(target_yaw - current_pos[2]);
    if (yaw_diff > M_PI) yaw_diff = 2.0*M_PI - yaw_diff;
    auto rot_time = std::chrono::duration_cast<Duration>(
      std::chrono::duration<double>(yaw_diff / angular_velocity));
    
    // Travel time
    auto travel_time = std::chrono::duration_cast<Duration>(
      std::chrono::duration<double>(distance / linear_velocity));
    
    current_time = current_time + rot_time + travel_time;
    
    // Final orientation
    double final_yaw = (i == path.size() - 1 && goal.orientation()) 
                       ? *goal.orientation() : target_yaw;
    
    // Update position
    Eigen::Vector3d new_pos;
    new_pos[0] = curr_loc.x();
    new_pos[1] = curr_loc.y();
    new_pos[2] = final_yaw;
    
    trajectory.insert(current_time, new_pos, Eigen::Vector3d::Zero());
    
    // Find approach lane (use full graph for lane lookup)
    std::vector<std::size_t> approach_lanes;
    for (std::size_t l = 0; l < full_graph.lanes.size(); ++l)
    {
      if (full_graph.lanes[l].entry().waypoint_index() == path[i-1] &&
          full_graph.lanes[l].exit().waypoint_index() == path[i])
      {
        approach_lanes.push_back(l);
        break;
      }
    }
    
    // Create Plan::Waypoint
    Plan::Checkpoints cp;
    cp.push_back(Plan::Checkpoint{0, static_cast<uint64_t>(i)});
    plan_waypoints.push_back(Plan::Waypoint::Implementation::make(
      new_pos, current_time, path[i],
      approach_lanes, std::vector<Plan::Progress>{},
      cp, nullptr, Dependencies{}
    ));
  ...
  // Create route
  routes.emplace_back(map_name, std::move(trajectory));
  
  return PlanData{
    std::move(routes),
    std::move(plan_waypoints),
    start,
    total_cost
  };
}
  ...
  std::size_t goal_vertex) const
{
  ...
  const double linear_velocity = _supergraph->traits().linear().get_nominal_velocity();
  
  for (const auto& start : start_options)
  {
    // Use cuOpt distance_engine for GPU-accelerated shortest path routing
    auto cuopt_result = CuOptRouter::find_shortest_path(
      start.waypoint(),
      goal_vertex,
      _supergraph->original()
    ...
    const auto cost_offset = [&]()
      {
        ...
        const auto speed = [&]()
          {
            const auto agent_speed = linear_velocity;
    ...
    // Create ForestSolution from cuOpt distance_engine result (GPU-accelerated routing)
    auto solution = std::make_shared<ForestSolution>();
    solution->cost = cuopt_result->total_cost / linear_velocity; // Convert distance to time
    solution->path = cuopt_result->path;
