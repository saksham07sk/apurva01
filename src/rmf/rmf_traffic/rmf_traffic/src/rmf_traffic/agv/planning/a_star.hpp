...
NodePtr a_star_search(
  ...
  SearchQueue& queue)
{
  // Timing tip: this function encapsulates the generic A* loop used by every
  // RMF path planner. Wrapping it with a stopwatch gives the definitive
  // “planning took X seconds” measurement for any pickup→dropoff task.
  while (!queue.empty())
  {
