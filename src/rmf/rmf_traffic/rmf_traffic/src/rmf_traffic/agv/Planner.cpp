...
{
  std::cout << "[Planner.cpp] 📥 PLAN REQUEST: plan() called with start waypoint " 
  << start.waypoint() << " and goal waypoint " << goal.waypoint() << std::endl;
...
{
  std::cout << "[Planner.cpp] 📥 PLAN REQUEST (with options): plan() called with start waypoint " 
  << start.waypoint() << " and goal waypoint " << goal.waypoint() << std::endl;
  return Result::Implementation::generate(
    _pimpl->interface,
    {start},
