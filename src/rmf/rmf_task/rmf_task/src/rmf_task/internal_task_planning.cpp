...
{
  std::cout << "[Candidates::make] ✅nineth call internal_task_planner.cpp" << std::endl;
  Map initial_map;
...
std::shared_ptr<PendingTask> PendingTask::make(
  ...
  TaskPlanner::TaskPlannerError& error)
{
  std::cout << "[PendingTask::make] ✅Eighth Call - internal_task_planning.cpp: " 
  << request_->booking()->id() << std::endl;
