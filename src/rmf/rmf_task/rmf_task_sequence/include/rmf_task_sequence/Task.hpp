...
public:
  ...
  // New overloaded function with lane closure support
  static rmf_task::Activator::Activate<Description> make_activator(
    Phase::ConstActivatorPtr phase_activator,
    std::function<rmf_traffic::Time()> clock,
    std::shared_ptr<void> lane_closure_publisher,
    std::function<void(std::shared_ptr<void>, const std::string&, const std::vector<int>&, const std::vector<int>&)> lane_closure_callback);
